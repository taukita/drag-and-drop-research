...
namespace DragAndDropResearch.Helpers
{
    internal static class CursorHelper
    {
        public static Cursor ConvertToCursor(FrameworkElement control, Point hotSpot)
        {
            // convert UIElement to PNG stream
            var pngStream = new MemoryStream();
            var rtb = new RenderTargetBitmap((int) control.ActualWidth, (int) control.ActualHeight, 96, 96,
            ...
            cursorStream.Write(new[] {(byte) 0x02, (byte) 0x00}, 0, 2);
                // ICONDIR: Specifies image type: 1 for icon (.ICO) image, 2 for cursor (.CUR) image. Other values are invalid
            cursorStream.Write(new[] {(byte) 0x01, (byte) 0x00}, 0, 2);
                // ICONDIR: Specifies number of images in the file.
            cursorStream.Write(new[] {(byte) control.ActualWidth}, 0, 1);
                // ICONDIRENTRY: Specifies image width in pixels. Can be any number between 0 and 255. Value 0 means image width is 256 pixels.
            cursorStream.Write(new[] {(byte) control.ActualHeight}, 0, 1);
                // ICONDIRENTRY: Specifies image height in pixels. Can be any number between 0 and 255. Value 0 means image height is 256 pixels.
            ...
            cursorStream.Write(new[] {(byte) hotSpot.X, (byte) 0x00}, 0, 2);
                // ICONDIRENTRY: Specifies the horizontal coordinates of the hotspot in number of pixels from the left.
            cursorStream.Write(new[] {(byte) hotSpot.Y, (byte) 0x00}, 0, 2);
